...
export default class HdtAccountSelectionFlow extends LightningElement {
    @api searchLabel;
    @api searchVariant;
    ...
    handleRecordSelection(event){
        if (this.concatenate === true) {
            console.log('Concatenazione di tutti i codici BP selezionati...');
            this.results = '';
            for (var selectedRow in event.detail.selectedRows) {
                this.results += event.detail.selectedRows[0].CustomerCode__c + '; ';
            }
        } else {
    ...
    handleNext(event){
        if(this.results != null && this.results != "" && this.results != "undefined"){
            ...
            this.showMessage('Errore','Attenzione! Seleziona un Account prima di andare avanti','error');  
        }

...
(
	function() {
	  TOOLBAR_LOAD_CALLBACK();
	}
  )();
  
  function TOOLBAR_LOAD_CALLBACK() {
	// Set AGENTDESKTOP Namespace in Window Global Scope
	window.TOOLBAR = (function () {
	  return {
		...
		Connect: function () {
		  try {
			  ...
			  window.TOOLBAR.SocketObj.on("connect",
				function(data) {
				  console.log("[ELECTRON TOOLBAR SDK] Socket.IO Connected to [wss://localhost:2337]");
				}
			  );
			  window.TOOLBAR.SocketObj.on("AGENTDESKTOP_CONTACT_CALLBACK", 
				function(data) {
				  var dataObj = null;
		...
		emit_Action: function (className, method, params) {
		  ...
		  var targetObject = {};
		  targetObject.action = "SDK_ACTION";
		...
		AGENT: (function () {
		  return {
			  ...
			  Logout: function() {
				  window.TOOLBAR.emit_Action( "X2L", "AD_Agent_Logout", new Array( ) );
			  },
			  ...
			  getAllAgentState: function () {
				  if ( typeof window.TOOLBAR.DeferredObject["getAllAgentState"] !== "undefined" ) {
					  delete window.TOOLBAR.DeferredObject["getAllAgentState"];
				  }
				  window.TOOLBAR.DeferredObject["getAllAgentState"] = window.TOOLBAR.JQ.Deferred();
				  window.TOOLBAR.emit_getParameter( "X2L", "getAllAgentState", new Array() );
				  return window.TOOLBAR.DeferredObject["getAllAgentState"].promise();
			  },
			  getAgentStateByID: function (AgentID) {
				  if ( typeof window.TOOLBAR.DeferredObject["getAgentStateByID"] !== "undefined" ) {
					  delete window.TOOLBAR.DeferredObject["getAgentStateByID"];
				  }
				  window.TOOLBAR.DeferredObject["getAgentStateByID"] = window.TOOLBAR.JQ.Deferred();
				  window.TOOLBAR.emit_getParameter( "X2L", "getAgentStateByID", new Array( AgentID ) );
				  return window.TOOLBAR.DeferredObject["getAgentStateByID"].promise();
			  },
		...
		CONTACT: (function () {
		  return {
			...
			bindContactCallback: function (callback) {
			  try {
				window.TOOLBAR.JQ(window).bind("AGENTDESKTOP_CONTACT_CALLBACK", callback);
			  } catch (err) {
				console.error(err);
			  }
			},
			unbindContactCallback: function (callback) {
			  try {
				window.TOOLBAR.JQ(window).unbind("AGENTDESKTOP_CONTACT_CALLBACK", callback);
			  } catch (err) {
				console.error(err);
			  }
			},
			triggerContactCallback : function (param) {
			  try {
				window.TOOLBAR.JQ(window).trigger( "AGENTDESKTOP_CONTACT_CALLBACK", [param] );
			  } catch (err) {
				console.error(err);
			  }            
			},
			  ...
			  try {
				...
				return "";
					  ...
					  getContactList: function () {
						  if ( typeof window.TOOLBAR.DeferredObject["getContactList"] !== "undefined") {
							  delete window.TOOLBAR.DeferredObject["getContactList"];
						  }
						  window.TOOLBAR.DeferredObject["getContactList"] = window.TOOLBAR.JQ.Deferred();
						  window.TOOLBAR.emit_getParameter( "X2L", "getContactList", new Array() );
						  return window.TOOLBAR.DeferredObject["getContactList"].promise();
						  
						  //window.TOOLBAR.emit_Action( "X2L", "getContactList", new Array( ) );
					  }
		  }; 
		}) ()
	  };
	...
	// BUILT-IN jQuery
	window.TOOLBAR.JQ = EmbedjQuery();
	if ( (typeof window.TOOLBAR.JQ !== "undefined") && (window.TOOLBAR.JQ !== null) ) {
	  if ( (typeof window.TOOLBAR.JQ.noConflict !== "undefined") && (window.TOOLBAR.JQ.noConflict !== null) ) {
		try {
		  window.TOOLBAR.JQ.noConflict();
		  window.TOOLBAR.JQ(document).on("ready",
			function() {
			  window.TOOLBAR.Connect();
			  /*
			  window.TOOLBAR.JQ.getScript("https://localhost:2337/socket.io/socket.io.js",
				function() {
				  window.TOOLBAR.Connect();
				}
			  );
